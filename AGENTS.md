...
- To confirm a dashboard/UI or API change actually works on real Windows, prompt the developer to install the freshly built MCP — `make msi` then `make install-msi` (the install needs a UAC prompt, so a human runs it) — and (re)start it. You are then connected to that running winctl-mcp instance, so verify your own change with its tools: open the dashboard window (`winctl-tray open-dashboard` / the launcher), capture a screenshot and/or a UIA snapshot of it, and read back the affected tab. winctl-mcp can inspect its own dashboard window — use that to close the visual-inspection loop instead of guessing from code.
