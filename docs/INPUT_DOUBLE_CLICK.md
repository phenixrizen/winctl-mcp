...
- `interval_ms`: optional interval between clicks.
- `fail_if_outside_bound`: optionally fail if preflight resolves outside the bound target.

## Notes

The bound window is revalidated before input is sent. Results include replay metadata for future macro promotion.
