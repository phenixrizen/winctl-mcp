...
Stopping joins the background capture worker, encodes the captured PNG frames into a GIF, and returns `recording.output_path`, `frames_dir`, `frame_count`, elapsed time, warnings, and target metadata.
